...
<script>
		...
		$.ajax({
			...
			success: alarmList => {
				if(alarmList.length){
							...
							'<div class="row mb-3">' +
				                '<div class="col">' +
				                    '<div class="alert alert-warning alert-dismissible fade show" role="alert">' +
				                        '<p class="text-center">' + alarm.alarmContent + '</a>' +
					...
					$('#alarms').append(alarms.join(''))
				} else $('#alarms').append(
					`<div class='row mb-3'>
                		<div class='col'>
		                    <h6 class='text-center'>알람이 없습니다.</h6>
        		        </div>
            		</div>`		
				)
			}
	...
	function meetingDelAlarm(){
		
		let alarm = {
			alarmContent: $('#meetingTitle').text() + '모임이 삭제되었습니다.',
